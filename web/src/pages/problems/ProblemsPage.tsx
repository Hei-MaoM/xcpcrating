...
/** Detail tags rendered inline before the rest collapse into a "+N" chip. */
const DETAIL_TAG_LIMIT = 4

/** Fine-grained algorithm tags; long lists stay on one line. */
function DetailTags({ tags, broad }: { tags: readonly string[]; broad: readonly string[] }) {
  // A detail tag that only repeats a coarse axis label adds no information.
  const unique = tags.filter((tag) => !broad.includes(tag))
  const shown = unique.slice(0, DETAIL_TAG_LIMIT)
  const rest = unique.slice(DETAIL_TAG_LIMIT)
  if (unique.length === 0) return null
  return (
    <div className="problem-tags problem-tags--detail" aria-label="具体算法">
      {shown.map((tag) => <span key={tag}>{tag}</span>)}
      {rest.length > 0 && (
        <span className="problem-tags__more" title={rest.join('、')}>+{rest.length}</span>
      )}
    </div>
  )
}
