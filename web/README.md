...
赛前预测、实际结果、选手题型七维雷达面板、表现分轨迹与逐场历史。
...
- 选手题型面板按 `all / official` 两种口径、`overall` 分档随分片下发；雷达图按**该维度的
  全场名次**绘制（第 1 名满格，前 1% 仍在外圈，其余按名次依次收小）
- 题型排行榜按七个轴分别生成一个文件，只请求当前题型；文件用 `rowFields` + 数组行的紧凑
  格式（每行 12 个字段，字段名只出现一次，体积约为对象行的一半），由 Web Worker 下载、
  解码、排名、搜索与分页，主线程只收到当前 30 行；首页空闲时会预热默认维度。
  1 道题也展示，分数向题型先验收缩，排名分与展示分分离；题型来源为 2023 年以来可匹配到 QOJ 的赛事
...
| `#/predictions` | 即将举行比赛的赛前预测列表 |
| `#/prediction/:slug` | 正式队伍的积分排序、奖牌排序与预计奖牌线 |
| `#/player/:key` | 选手详情（题型七维面板 + 表现分轨迹 + 逐场历史） |
