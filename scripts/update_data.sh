...
# ---------------------------------------------------------------------------- #
# 1b. 叠加本仓库自带的 srk（上游尚未收录的网络预选赛，见 srk-extra/）
#     多数网络预选赛已随上游 PR #63 进入数据源；srk-extra 现仅保留上游仍缺的那些
#     （当前是 2025 EC / CCPC 网络赛），每次更新后覆盖进去，保证本地与 CI
#     （pages.yml 同样的 cp 步骤）导出口径一致。上游补齐后即可删除对应文件。
# ---------------------------------------------------------------------------- #
