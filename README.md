...
│   ├── panel_metrics.py        # 题型十三维面板用的单场提交 / 时间指标
│   ├── problem_types.py        # QOJ 题型 manifest 校验与题型掌握度聚合
│   ├── skill_panel.py          # 题型十三维面板与原始能力分排名
│   ├── skill_sequential.py     # 题型能力的时序（逐场）更新
...
- 收录 `icpc / ccpc / provincial` 三类官方榜单；至少有 1 行带有效队员名单的比赛进入积分回放。
  整场无名单的比赛仍导出赛事档案，可查看队伍、名次、过题数和罚时，个人评分字段留空。
- **同场去重**：同一物理比赛的双榜（如邀请赛 + 省赛切片，同日、队员高度重叠）只保留一份。
  网络预选赛是全国性资格赛、与同期线下赛队员大量重叠，**不参与去重**（按区域赛单独计分）。

题型十三维按比赛时间顺序更新绝对能力；公开分数和排名直接使用与题目 rating 同尺度的原始值，
不转换为百分制，不在 1000 或 3000 处截断。雷达图改按**该维度的全场名次**绘制：第 1 名满格，
前 1% 仍落在最外圈，其余维度按名次依次收小（网格圈对应第 1 名 / 前 1% / 前 5% / 前 20% 名次带，图上不标数值）。
